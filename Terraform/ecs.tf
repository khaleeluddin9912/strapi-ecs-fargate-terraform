  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = { command = ["CMD-SHELL", "wget -qO- http://localhost:1337/ || exit 1"]; interval = 30; timeout = 10; retries = 3; startPeriod = 90 }
      environment = [
      ...
      logConfiguration = { logDriver = "awslogs"; options = { awslogs-group = aws_cloudwatch_log_group.strapi.name; awslogs-region = "ap-south-1"; awslogs-stream-prefix = "ecs" } }
    }
  ])
}
